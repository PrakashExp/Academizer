...
var AdminStuff = {
  ...
  updateFaculty: function(req, res, next) {
    ...
    var dept = mongoose.createConnection("localhost:27017/" + deptName);
    var Faculty = dept.model("faculty", DeptSchema.faculty);
    Faculty.findOneAndUpdate({ _id: facId }, data, { upsert: true }, function(
      err,
      results
    ) {
      if (err) throw err;
  ...
  addAchievement: function(req, res, next) {
    ...
    var dept = mongoose.createConnection("localhost:27017/" + deptName);
    var Faculty = dept.model("faculty", DeptSchema.faculty);
    Faculty.update({ _id: id }, { $pull: { achievement: delData } }, function(
      err,
      results
    ) {
      Faculty.update({ _id: id }, { $push: { achievement: data } }, function(
        err,
        results
      ) {
        if (err) throw err;
  ...
  deleteAchievement: function(req, res, next) {
    ...
    var dept = mongoose.createConnection("localhost:27017/" + deptName);
    var Faculty = dept.model("faculty", DeptSchema.faculty);
    Faculty.update({ _id: id }, { $pull: { achievement: data } }, function(
      err,
      results
    ) {
      if (err) throw err;
