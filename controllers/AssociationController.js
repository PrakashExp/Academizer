...
var AdminStuff = {
	...
	editAssoc : function(req, res, next){
		var dept = mongoose.createConnection('localhost:27017/'+req.body.nameInDb);
        var Basic = dept.model("basicInfo", AssocSchema.basic);
        var updatedRecords = {
        	description : req.body.desc
        }
        Basic.findOneAndUpdate({ "nameInDb" : req.body.nameInDb }, updatedRecords, { upsert : true }, function(err, results){
            if(err) {
                res.status(404).send({
    ...
    editAssocMember : function(req, res, next){
        ...
        Faculty.findOneAndUpdate({ _id : req.body.id }, data, { upsert : true }, function(err, results){
            if(err) throw err;
