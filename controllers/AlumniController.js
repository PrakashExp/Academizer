...
var alumniStuff = {
  ...
  editSelectedAlumni: function(req, res, next) {
    ...
    var database = mongoose.createConnection("localhost:27017/alumni"); //Connection
    var Alumni = database.model("alumniModel", AlumniSchema.alumniModel); //Alumni Model
    Alumni.findOneAndUpdate({ _id: id }, data, { upsert: true }, function(
      err,
      results
    ) {
      if (err)
  ...
  sendMail: function(to, subject, html) {
    ...
    var mailOptions = {
      from:
        '"St. Mary\'s Authority - Alumni Registeration" <smconline.authority@gmail.com>',
      to: "<" + to + ">",
      subject: subject,
      html: html,
      auth: {
        ...
        refreshToken:
          "1/-D3NW2unyQUJcEOdVGsEVI0LMiQRmpuqv63H0eQeloIjqH9x_OrlnygegVd_9_ge",
        accessToken:
          "ya29.GltjBdt5zj9orMORrUsOWOvAVXlKiuP4wZcjpbn_Wg-rnyuxr5xqfYwNNCFOSd5dVrbDcKDLyRtDD1-f3IkTPXs-gYAwTU2kygTBceGFRw16gBVm5uM5c10dvi55",
  ...
  addAlumniNotification: function(req, res, next) {
    var database = mongoose.createConnection("localhost:27017/alumni"); //Connection
    var Alumni = database.model(
      "alumniNotification",
      AlumniSchema.alumniNotification
    ); //Alumni Model
